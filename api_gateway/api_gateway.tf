...
resource "aws_api_gateway_integration" "order_integration" {
  ...
  type                    = "HTTP"
  uri                     = "http://${var.cluster_endpoint}/fast-food-order"
}
...
resource "aws_api_gateway_integration" "preparation_integration" {
  ...
  integration_http_method = "POST"
  type                    = "HTTP"
  uri                     = "http://${var.cluster_endpoint}/fast-food-preparation"
}
...
resource "aws_api_gateway_integration" "payment_integration" {
  ...
  integration_http_method = "POST"
  type                    = "HTTP"
  uri                     = "http://${var.cluster_endpoint}/fast-food-payment"
}
...
output "api_gateway_url" {
  value = aws_api_gateway_deployment.fast_food_api_deployment.invoke_url
}
